...
const LOG_FILE_PATH = path.join(__dirname, "debug.log"); // file path to log
const LOG_FILESTREAM = fs.createWriteStream(LOG_FILE_PATH, {flags : "w"}); // creates object to file

console.log = (v) => {
    LOG_FILESTREAM.write(util.format(v) + "\n");  // makes a new file to log
    process.stdout.write(until.format(v) + "\n"); // still logs in terminal
}
